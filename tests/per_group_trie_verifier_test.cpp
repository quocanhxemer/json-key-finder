...
TEST(TeddyPerGroupTrieVerifierTest, ChecksOnlyCandidateGroups) {
    ...
    const teddy::CandidateResult no_candidates =
        teddy::verify_json_key_candidate(data, 6, 0, verifier);
    ...
    EXPECT_EQ(no_candidates.type, teddy::CANDIDATE_KEY_NOT_FOUND);
}

TEST(TeddyPerGroupTrieVerifierTest, ChecksEveryCandidateGroupUntilAMatch) {
    constexpr std::string_view data = R"("beta":1)";
    const std::vector<std::string_view> keys = {"alpha", "beta"};
    const findkey_teddy_config config = FINDKEY_TEDDY_CONFIG_INIT;
    const teddy::CompilationData compilation = teddy::compile(keys, config);
    const teddy::VerificationBuildContext context{keys, compilation};
    ...
    const uint8_t beta_group =
        group_bit_for_suffix(compilation, compilation.key_suffix_ids[1]);
    const uint8_t both_groups = alpha_group | beta_group;

    const teddy::CandidateResult result =
        teddy::verify_json_key_candidate(data, 5, both_groups, verifier);

    ASSERT_EQ(result.type, teddy::CANDIDATE_MATCH);
    EXPECT_EQ(result.position, 1u);
    EXPECT_EQ(result.key_id, 1u);
}
...
TEST(TeddyPerGroupTrieVerifierTest,
    ...
    const uint8_t x_group =
        group_bit_for_suffix(compilation, compilation.key_suffix_ids[0]);
    const uint8_t abc_group =
        group_bit_for_suffix(compilation, compilation.key_suffix_ids[1]);
