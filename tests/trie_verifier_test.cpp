...
TEST(TeddyTrieVerifierTest, ReturnsTheCorrectKey) {
    ...
    const teddy::CandidateResult result =
        findkey_test::verify_candidate<teddy::TrieVerifier>(data, 6, keys);
...
TEST(TeddyTrieVerifierTest, ReportsMalformedJsonContext) {
    ...
    for (const auto& test_case : cases) {
        ...
        const teddy::CandidateResult result =
            findkey_test::verify_candidate<teddy::TrieVerifier>(
...
TEST(TeddyTrieVerifierTest, RecognizesAKeyThatIsASuffixOfAnother) {
    ...
    const teddy::CandidateResult short_key =
        findkey_test::verify_candidate<teddy::TrieVerifier>(R"("id":1)", 3,
                                                            keys);
    const teddy::CandidateResult long_key =
        findkey_test::verify_candidate<teddy::TrieVerifier>(R"("user_id":1)", 8,
                                                            keys);
